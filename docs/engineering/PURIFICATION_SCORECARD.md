...
| Legacy routes | 11 routes with no UI consumer + 1 retired (`/api/tasks/execute`) · 3 orphan pages. P2-G inventory (2026-09-15): 7 no-consumer candidates + 1 consolidation (G01–G08) | `/api/stream` retired to a `withAuth` 410 (1,065 → 50 lines; G-B1) | shared retired-route guard covers `/api/tasks/execute` and `/api/stream` | G02 `/api/agents/execute`, G03 `/api/action`, G04 `/api/knowledge/search` (G-B2, G-B3); G05 `/api/files/analyze`, G06 `/api/files/upload`, G07 voice (founder decisions); G08 `/api/agents` model; 3 orphan pages | P2-G01 |
